...
// Initialize metrics pusher after server starts
async function initializeMetricsPusher() {
  // Wait for server to be ready
  setTimeout(async () => {
    try {
      const fs = require('fs')
      const configPath = '/app/config/dasharr-settings.json'
      
      // Check if config file exists and push is enabled
      if (fs.existsSync(configPath)) {
        const configData = fs.readFileSync(configPath, 'utf-8')
        const settings = JSON.parse(configData)
        
        // Only initialize if push is explicitly enabled
        if (settings.enablePush === true && settings.pushTarget && settings.pushSecret) {
          console.log('Initializing metrics pusher...')
          
          const response = await fetch(`http://${hostname}:${currentPort}/api/metrics/pusher/init`, {
            method: 'POST',
            headers: {
          ...
          if (result.success) {
            console.log('✓ Metrics pusher initialized successfully')
          }
        }
      }
    } catch (error) {
      // Silently handle errors - metrics push is optional
    }
...
startServer({
  ...
  console.log(`> Server listening on http://${hostname}:${currentPort}`);
  // Initialize metrics pusher after server is ready
  initializeMetricsPusher();
}).catch((err) => {
  console.error(err);
  process.exit(1);
});